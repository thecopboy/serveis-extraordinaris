...
scp clouding/docker-compose.production.yml $SSH_USER@$SERVER_IP:/opt/serveis-extraordinaris/docker-compose.yml
scp clouding/.env.production $SSH_USER@$SERVER_IP:/opt/serveis-extraordinaris/.env
scp schema.sql $SSH_USER@$SERVER_IP:/opt/serveis-extraordinaris/schema.sql
scp clouding/backup.sh $SSH_USER@$SERVER_IP:/opt/serveis-extraordinaris/backup.sh
scp clouding/cleanup_tokens.sh $SSH_USER@$SERVER_IP:/opt/serveis-extraordinaris/cleanup_tokens.sh
...
# Configurar cron
(crontab -l 2>/dev/null; echo "0 3 * * * /opt/serveis-extraordinaris/backup.sh >> /var/log/serveis-backup.log 2>&1") | crontab -
(crontab -l 2>/dev/null; echo "0 4 * * * /opt/serveis-extraordinaris/cleanup_tokens.sh >> /var/log/serveis-cleanup.log 2>&1") | crontab -
